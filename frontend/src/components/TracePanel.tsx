...
export interface TracePanelProps {
   ...
   * container (the pipeline modal) — no outer card chrome, no duplicate
          ...
          >
            ...
            <article className="trace-stage__card mb-1 min-w-0 rounded-2xl border border-[#3c507e] bg-[#14224f]/88 p-3.5 transition-colors hover:border-[#4c619a] hover:bg-[#16275a]">
              ...
              <details className="trace-stage__output mt-3" open={index === stages.length - 1}>
                ...
                <pre
                  id={outputId}
                  className="mt-2 max-h-80 overflow-x-auto overflow-y-auto whitespace-pre break-normal rounded-xl border border-[#304374] bg-[#09132e] p-3 font-mono text-[0.76rem] leading-5 text-[#d8def0]"
                >
...
export function TracePanel({
  ...
  if (embedded) {
    return (
      <div className={joinClasses("trace-panel-embedded", className)}>
        {stages.length > 0 ? (
          <div className="trace-panel-embedded__summary flex flex-wrap items-center gap-x-4 gap-y-2 rounded-2xl border border-[#3c507e] bg-[#14224f]/70 px-4 py-3 font-mono">
            <span className="text-base font-semibold text-[#f5f1ea]">
              {stages.length}/{EXPECTED_STAGE_COUNT} stages
            </span>
            <span className="text-base font-semibold text-[#f5f1ea]">
              Total: {formatLatency(totalLatency)}
            </span>
            <span className="ml-auto inline-flex items-center gap-1.5 rounded-full border border-[#4c619a] bg-[#0c1733] px-2.5 py-1 text-[0.72rem] font-bold uppercase tracking-[0.1em] text-[#e3c58a]">
              <Activity aria-hidden="true" size={12} />
              {isLoading ? "Processing" : traceStatus}
            </span>
          </div>
        ) : null}
        <div className="trace-panel-embedded__body mt-4">
          <TraceTimeline stages={stages} isLoading={isLoading} unavailableMessage={unavailableMessage} />
        </div>
      </div>
    );
  }
